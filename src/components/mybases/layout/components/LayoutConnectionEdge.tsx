...
const LayoutConnectionEdge = memo((props: EdgeProps) => {
    ...
    return (
        <>
            ...
            <path d={edgePath} fill="none" stroke="transparent" strokeWidth={EDGE_HIT_WIDTH} className="react-flow__edge-interaction" />
            {/* Selection highlight glow */}
            {selected && <path d={edgePath} fill="none" stroke="#38bdf8" strokeWidth={8} strokeOpacity={0.4} />}
            {/* Visible path */}
            <path d={edgePath} fill="none" stroke={selected ? "#38bdf8" : strokeColor} strokeWidth={selected ? 3 : 2} markerEnd={typeof markerEnd === "string" ? markerEnd : undefined} />
            <EdgeLabelRenderer>
                {labelPositions.map((pos, i) => (
                    ...
                    >
                        <span className="text-[10px] font-mono px-1 rounded bg-base-200/80 text-base-content/90 select-none cursor-pointer" onDoubleClick={handleRemove} title="Double-click to remove">
                            {labelText}
                        </span>
                    </div>
                ))}
