...
import {
  readEnergyGroupsFromStorage,
  writeEnergyGroupsToStorage,
} from "./energy-groups-storage";
...
regCoeffect(EFFECT_IDS.GET_DATA_VERSION, (coeffects) => {
  coeffects.localStoreDataVersion = localStorage.getItem(
    "dataVersion",
  ) as DataVersion | null;
  return coeffects;
});
...
regEffect(EFFECT_IDS.SET_BASES, (bases: Base[]) => {
  ...
  pendingBasesPersistTimer = setTimeout(
    flushPendingBasesPersist,
    BASES_PERSIST_DEBOUNCE_MS,
  );
});
