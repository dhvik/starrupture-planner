...
import {
  ...
  isAmplifierBuilding,
  getCoreLevels,
} from "../components/mybases/utils/baseCoreUtils";
...
regSub(
  SUB_IDS.PLANNER_SELECTED_CORPORATION_LEVEL,
  "plannerSelectedCorporationLevel",
);
...
regSub(SUB_IDS.BASES_LAYOUT_OPEN_DIRECTLY, "baseLayoutOpenDirectly");
...
regSub(
  SUB_IDS.ITEMS_AVAILABLE_PRODUCTION_BUILDINGS,
...
regSub(
  ...
  () => [
    [SUB_IDS.ITEMS_SELECTED_CATEGORY],
    [SUB_IDS.ITEMS_SELECTED_BUILDING],
    [SUB_IDS.ITEMS_SEARCH_TERM],
    [SUB_IDS.ITEMS_LIST],
    [SUB_IDS.BUILDINGS_LIST],
  ],
);
...
regSub(
  SUB_IDS.ITEMS_RECIPES_BY_INPUT_ITEM_ID,
  (
    buildings: DbBuilding[],
    itemId: string,
  ): { recipe: Recipe; building: DbBuilding }[] => {
    if (!itemId) return [];
...
regSub(
  ...
  (buildings: DbBuilding[], helperMaps: ItemsHelperLookups): DbBuilding[] => {
    ...
    const sorted = [...productionBuildings].sort((a, b) => {
      ...
      const minLevelA =
        usageA.length > 0 ? Math.min(...usageA.map((u) => u.level)) : Infinity;
      const minLevelB =
        usageB.length > 0 ? Math.min(...usageB.map((u) => u.level)) : Infinity;
    ...
    const buildingsById = new Map<string, DbBuilding>(
...
regSub(
  SUB_IDS.CORPORATIONS_STATS_SUMMARY,
  (corporationsWithStats: CorporationWithStats[]) => {
    return {
      ...
      totalLevels: corporationsWithStats.reduce(
        (total: number, corp) => total + corp.stats.totalLevels,
        0,
      ),
      totalCost: corporationsWithStats.reduce(
        (total: number, corp) => total + corp.stats.totalCost,
        0,
      ),
    };
  },
...
regSub(
  SUB_IDS.PLANNER_AVAILABLE_CORPORATION_LEVELS,
  (
...
regSub(
  SUB_IDS.PLANNER_PRODUCTION_FLOW,
  (
    selectedItem: string | null,
    targetAmount: number,
    buildings: DbBuilding[],
  ...
  () => [
    [SUB_IDS.PLANNER_SELECTED_ITEM_ID],
    [SUB_IDS.PLANNER_TARGET_AMOUNT],
    [SUB_IDS.BUILDINGS_LIST],
    [SUB_IDS.PLANNER_SELECTED_CORPORATION_LEVEL],
    [SUB_IDS.PLANNER_RECIPE_SELECTIONS],
  ],
);

regSub(
  SUB_IDS.PLANNER_RECIPE_OPTIONS,
  (
    selectedItem: string | null,
...
regSub(
  SUB_IDS.PLANNER_STATS_SUMMARY,
  (
    selectedItem: string | null,
    productionFlow: ProductionFlowResult,
  ): { totalBuildings: number; totalEnergy: number; totalHotness: number } => {
    if (!selectedItem || !productionFlow || productionFlow.nodes.length === 0) {
      return { totalBuildings: 0, totalEnergy: 0, totalHotness: 0 };
    }
    const totalBuildings = productionFlow.nodes.reduce(
      (sum, node) => sum + Math.ceil(node.buildingCount),
      0,
    );
    const totalEnergy = productionFlow.nodes.reduce(
      (sum, node) => sum + node.totalPower,
      0,
    );
    const totalHotness = productionFlow.nodes.reduce(
...
regSub(
  ...
  (
    ...
    const buildingStats = Array.from(buildingMap.values()).sort(
      (a, b) => b.count - a.count,
    );
    ...
    const totalBuildings = nodes.reduce(
      (sum, node) => sum + Math.ceil(node.buildingCount),
      0,
    );
    ...
    const itemsWithData = Array.from(itemsUsed).map((itemId) => {
...
regSub(
  SUB_IDS.PLANNER_SELECTABLE_ITEMS,
  (items: Item[]): Item[] => {
    return items
...
//============================================================
regSub(
  SUB_IDS.ENERGY_GROUPS_BY_ID_MAP,
...
function calculateBaseDetailStats(
  ...
  const pooled =
    allBases && energyGroupsById
      ? calculatePooledEnergy(base, allBases, buildingsById, energyGroupsById)
      : null;
  const effectiveGeneration = pooled
    ? pooled.pooledGeneration
    : energyGeneration;
  const effectiveConsumption = energyConsumption;
  const energyGridConsumption = pooled
    ? pooled.pooledConsumption
    : energyConsumption;
  ...
  const heatPercentage = Math.min(
    (totalHeat / baseCoreHeatCapacity) * 100,
    100,
  );
  ...
  const isEnergyInsufficient =
    hasEnergyLoad &&
    (effectiveGeneration === 0 || effectiveConsumption > effectiveGeneration);

  // Energy group info
  const energyGroupId = base.energyGroupId;
  const energyGroupName =
    energyGroupId && energyGroupsById
...
regSub(
  ...
  (
    ...
    energyGroupsById: Record<string, EnergyGroup>,
    allBases: Base[],
  ): BaseDetailStats | null => {
    if (!selectedBase) return null;
    return calculateBaseDetailStats(
      selectedBase,
      buildingsById,
      energyGroupsById,
      allBases,
...
regSub(
  ...
  (
    ...
    return collectConfiguredSectionItems(
      base,
      buildingsById,
      itemsMap,
      "inputs",
...
regSub(
  SUB_IDS.BASES_BUILDING_SECTION_BUILDINGS,
  (
    base: Base | null,
    buildingsById: BuildingsByIdMap,
    _baseId: string,
...
regSub(
  SUB_IDS.BASES_BUILDING_SECTION_STATS,
  (
    basesById: BasesById,
    buildingsById: BuildingsByIdMap,
    baseId: string,
    ...
    const baseBuildings = base.buildings.filter(
...
regSub(
  SUB_IDS.BASES_AVAILABLE_BUILDINGS_FOR_SECTION,
...
regSub(
  ...
  (bases: Base[], buildingsById: BuildingsByIdMap): MyBasesStats => {
    ...
    bases.forEach((base: Base) => {
      ...
      totalHeatCapacity += calculateBaseCoreHeatCapacity(
        base.coreLevel ?? 0,
        base.buildings,
        buildingsById,
      );
    ...
    const isEnergyInsufficient =
      totalEnergyUsed > 0 &&
      (totalEnergyProduced === 0 || totalEnergyUsed > totalEnergyProduced);

    const totalPlans = bases.reduce(
      (sum, base) => sum + (base.productions?.length ?? 0),
      0,
    );
    return {
...
// Connection transfer rates - per-connection current/max throughput
regSub(
  SUB_IDS.BASES_LAYOUT_CONNECTION_TRANSFER_RATES,
  (
    result: ReturnType<typeof calculateLayoutBalance>,
    layout: BaseLayout | undefined,
  ): Record<string, ConnectionTransferRate> => {
    const rates = calculateConnectionTransferRates(
      layout,
      result.buildingStates,
    );
    const obj: Record<string, ConnectionTransferRate> = {};
    rates.forEach((v, k) => {
      obj[k] = v;
    });
    return obj;
...
//============================================================
regSub(
  SUB_IDS.PRODUCTION_PLAN_SECTION_IDS,
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_SECTION_ENTITY_BY_ID,
  (
    base: Base | null,
    _baseId: string,
    sectionId: string,
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_SECTION_FLOW_BY_ID,
  (
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_MODAL_RECIPE_OPTIONS,
  (
    modalState: CreateProductionPlanModalState,
    productionFlow: ProductionFlowResult,
    buildings: DbBuilding[],
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_MODAL_AVAILABLE_CORPORATION_LEVELS,
  (
    corporations: Corporation[],
    modalState: CreateProductionPlanModalState,
  ): CorporationLevelInfo[] => {
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_SECTION_STATS_BY_ID,
  (
    productionFlow: ProductionFlowResult,
    section: Production | null,
  ): ProductionPlanSectionStats => {
    ...
    const totalHeat = productionFlow.nodes.reduce(
      (sum, node) => sum + node.totalHeat,
      0,
    );
    const totalPower = productionFlow.nodes.reduce(
      (sum, node) => sum + node.totalPower,
      0,
    );
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_SECTION_VIEW_MODEL_BY_ID,
  (
    section: Production | null,
    base: Base | null,
    itemsMap: Record<string, Item>,
    ...
    const corporationName = section.corporationLevel
      ? corporations.find(
          (c) => c.id === section.corporationLevel?.corporationId,
        )?.name || null
      : null;
    ...
    requiredBuildings.forEach(({ buildingId, count }) => {
      ...
      const occupied =
        occupancyFromOtherPlans.occupiedBuildingTypeCounts.get(buildingId) || 0;
    ...
    if (sectionInputs.length > 0) {
      sectionInputs.forEach((inputBuilding: BaseBuilding) => {
        ...
        const item = inputBuilding.selectedItemId
          ? itemsMap[inputBuilding.selectedItemId]
          : null;
    ...
    const sharedInputShortages: SharedInputShortage[] =
      calculateSharedInputShortages(base, section.id, buildings).map(
        (shortage) => {
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_SECTION_ITEM_NAME_BY_ITEM_ID,
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_MODAL_OPEN_STATE,
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_MODAL_HEADER_DATA,
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_MODAL_FORM_VALUES,
  (
    modalState: CreateProductionPlanModalState,
    items: Item[],
  ): {
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_MODAL_INPUT_SELECTOR_DATA,
  (
    basesById: BasesById,
    buildingsById: BuildingsByIdMap,
    itemsMap: Record<string, Item>,
    ...
    const inputItems = collectConfiguredSectionItems(
      base,
      buildingsById,
      itemsMap,
      "inputs",
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_MODAL_SELECTED_ITEM_ID,
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_MODAL_RAW_MATERIAL_DEFICITS,
  (
    productionFlow: ProductionFlowResult,
    items: Item[],
  ): RawMaterialDeficitWithName[] => {
...
regSub(
  SUB_IDS.PRODUCTION_PLAN_MODAL_FORM_VALIDITY,
  (
    modalState: CreateProductionPlanModalState,
