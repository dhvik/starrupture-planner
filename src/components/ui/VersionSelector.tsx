...
const VersionSelector: React.FC<VersionSelectorProps> = ({
  ...
  const currentVersion = useSubscription<DataVersion>([
    SUB_IDS.APP_DATA_VERSION,
  ]);
  const dataVersions = useSubscription<{ id: DataVersion; label: string }[]>([
    SUB_IDS.APP_DATA_VERSIONS,
  ]);
  const loadPending = useSubscription<boolean>([
    SUB_IDS.UI_GAME_DATA_LOAD_PENDING,
  ]);
