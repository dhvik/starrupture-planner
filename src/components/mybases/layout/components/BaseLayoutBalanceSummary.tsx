            ...
            {imbalancedItems.map((item) => {
              ...
              return (
                <tr key={item.itemId}>
                  ...
                  <td className="text-right">{item.totalProduction.toFixed(1)}/min</td>
                  <td className="text-right">{item.totalDemand.toFixed(1)}/min</td>
                  <td className={`text-right font-bold ${balanceClass}`}>
                    {balanceValue > 0 && '+'}
