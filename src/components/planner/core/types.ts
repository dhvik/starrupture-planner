...
export interface PlannerRecipeOption {
  key: string; // `output:${buildingId}:${outputItemId}` (legacy: `${buildingId}:${recipeIndex}`)
  buildingId: string;
  buildingName: string;
  recipeIndex: number;
  outputRate: number;
...
export interface ProductionFlowParams {
  ...
  /** Optional per-output recipe override map: output item id -> `output:${buildingId}:${outputItemId}` (legacy: `${buildingId}:${recipeIndex}`) */
